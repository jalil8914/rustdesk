...
fn elevate_install(
    ...
    let prompt = format!("{} wants to install then plugin", crate::get_app_name());
    crate::platform::elevate(args, &prompt)
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        &format!("{} wants to uninstall the plugin", crate::get_app_name()),
    )
}
